...
(define GOO-IMG (bitmap "assets/goo.gif"))
(define SEG-IMG (bitmap "assets/body.gif"))
(define HEAD-IMG (bitmap "assets/head.gif"))
...
(define (position-move p dx dy)
  (position (+ (position-x p) dx)
            (+ (position-y p) dy)))
...
(define (img-list+scene positions img scene)
  (cond [(empty? positions) scene]
        [else (img+scene (first positions) img
              (img-list+scene (rest positions) img scene))]))

(define (img+scene position img scene)
  (place-image img
               (* (position-x position) SEG-SIZE)
               (* (position-y position) SEG-SIZE)
               scene))

(define (goo-list+scene goos scene)
  (define (get-positions-from-goo goos)
    (cond [(empty? goos) empty]
          [else (cons(goo-location (first goos))
                  (get-positions-from-goo (rest goos)))]))
  (img-list+scene (get-positions-from-goo goos) GOO-IMG scene))
